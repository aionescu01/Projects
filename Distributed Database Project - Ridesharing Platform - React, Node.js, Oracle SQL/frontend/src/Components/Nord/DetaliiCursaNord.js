...
const DetaliiCursaNordManagement = () => {
  ...
  const fetchDetaliiCurse = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/nord/detaliiCursaNord');
  ...
  const handleSubmit = async () => {
    try {
      const url = selectedDetaliu 
        ? `http://localhost:3001/api/nord/detaliiCursaNord/${selectedDetaliu.cod_cursa}`
        : 'http://localhost:3001/api/nord/detaliiCursaNord';
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`http://localhost:3001/api/nord/detaliiCursaNord/${id}`, {
        method: 'DELETE',
      });
...
export default DetaliiCursaNordManagement;