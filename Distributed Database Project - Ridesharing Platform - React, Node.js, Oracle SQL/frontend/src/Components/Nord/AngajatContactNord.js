...
const AngajatContactNordManagement = () => {
  ...
  const fetchEmployees = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/nord/angajatContactNord');
  ...
  const handleSubmit = async () => {
    try {
      const url = selectedEmployee 
        ? `http://localhost:3001/api/nord/angajatContactNord/${selectedEmployee.cod_angajat}`
        : 'http://localhost:3001/api/nord/angajatContactNord';
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`http://localhost:3001/api/nord/angajatContactNord/${id}`, {
        method: 'DELETE',
      });
...
export default AngajatContactNordManagement;