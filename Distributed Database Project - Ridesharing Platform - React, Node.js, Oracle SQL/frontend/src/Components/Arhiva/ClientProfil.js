...
const ClientProfilManagement = () => {
  ...
  const fetchClients = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/arhiva/clientProfil');
  ...
  const handleSubmit = async () => {
    try {
      const url = selectedClient 
        ? `http://localhost:3001/api/arhiva/clientProfil/${selectedClient.cod_client}`
        : 'http://localhost:3001/api/arhiva/clientProfil';
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`http://localhost:3001/api/arhiva/clientProfil/${id}`, {
        method: 'DELETE',
      });
...
export default ClientProfilManagement;