...
const ClientContactSudManagement = () => {
  ...
  const fetchClients = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/sud/clientContactSud');
  ...
  const handleSubmit = async () => {
    try {
      const url = selectedClient 
        ? `http://localhost:3001/api/sud/clientContactSud/${selectedClient.cod_client}`
        : 'http://localhost:3001/api/sud/clientContactSud';
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`http://localhost:3001/api/sud/clientContactSud/${id}`, {
        method: 'DELETE',
      });
...
export default ClientContactSudManagement;