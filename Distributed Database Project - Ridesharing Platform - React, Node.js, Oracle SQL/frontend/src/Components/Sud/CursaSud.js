...
const CurseSudManagement = () => {
  ...
  const fetchCurse = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/sud/cursaSud');
  ...
  const handleSubmit = async () => {
    try {
      const url = selectedCursa 
        ? `http://localhost:3001/api/sud/cursaSud/${selectedCursa.cod_cursa}`
        : 'http://localhost:3001/api/sud/cursaSud';
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`http://localhost:3001/api/sud/cursaSud/${id}`, {
        method: 'DELETE',
      });
...
export default CurseSudManagement;