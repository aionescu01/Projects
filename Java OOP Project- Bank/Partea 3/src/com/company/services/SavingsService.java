...
public class SavingsService {
    ...
    public void loadFromCSV() {
        AccountService accountService = AccountService.getInstance();
        try{
            var columns = SavingsService.getCSVColumns("src/com/company/data/saving_accounts.csv");
            for(var fields : columns){

                Account a = accountService.getAccountById(Integer.parseInt(fields[0]));

                var newAccount = new Savings(
                        a,
                        Integer.parseInt(fields[1]),
                        Double.parseDouble(fields[2])
                );
                Savings_accts.add(newAccount);
    ...
    public void dumpToCSV(){
        try{
            var writer = new FileWriter("src/com/company/data/saving_accounts.csv");
            for(var customer : this.Savings_accts){
