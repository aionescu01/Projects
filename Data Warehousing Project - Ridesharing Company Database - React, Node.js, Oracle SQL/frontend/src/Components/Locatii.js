...
const LocatiiManagement = () => {
  ...
  const fetchLocatii = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/oltp/locatii');
  ...
  const handleSubmit = async () => {
    try {
      const url = selectedLocatie 
        ? `http://localhost:3001/api/oltp/locatii/${selectedLocatie.cod_locatie}`
        : 'http://localhost:3001/api/oltp/locatii';
  ...
  const handleDelete = async (cod_locatie) => {
    try {
      await fetch(`http://localhost:3001/api/oltp/locatii/${cod_locatie}`, {
        method: 'DELETE',
      });
...
export default LocatiiManagement;